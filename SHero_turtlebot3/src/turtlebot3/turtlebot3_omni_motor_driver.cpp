...
bool Turtlebot3MotorDriver::readEncoder(int32_t &front_value, int32_t &left_value, int32_t &right_value)
{
  ...
  // Set parameter
  dxl_addparam_result = groupSyncReadEncoder_->addParam(first_wheel_id_);
  if (dxl_addparam_result != true)
    return false;

  dxl_addparam_result = groupSyncReadEncoder_->addParam(second_wheel_id_);
  if (dxl_addparam_result != true)
    return false;
    
  dxl_addparam_result = groupSyncReadEncoder_->addParam(third_wheel_id_);
  if (dxl_addparam_result != true)
    return false;
  ...
  dxl_getdata_result = groupSyncReadEncoder_->isAvailable(second_wheel_id_, ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION);
  if (dxl_getdata_result != true)
    return false;
    
  dxl_getdata_result = groupSyncReadEncoder_->isAvailable(third_wheel_id_, ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION);
  if (dxl_getdata_result != true)
    return false;

  // Get data
  front_value  = groupSyncReadEncoder_->getData(first_wheel_id_,  ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION);
  left_value = groupSyncReadEncoder_->getData(second_wheel_id_, ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION);
  right_value = groupSyncReadEncoder_->getData(third_wheel_id_, ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION);
